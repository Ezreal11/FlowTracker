 ...
 */
public class MyDebuggerManagerListener extends MyListener implements DebuggerManagerListener {
    public MyDebuggerManagerListener(Persistence persistence) {
    ...
    private void onSessionChanged(long time, String when, DebuggerSession session) {
        ...
        e.addExtraData("project", StringUtil.md5Encode(projectName));
        e.addExtraData("session", StringUtil.md5Encode(sessionName));
